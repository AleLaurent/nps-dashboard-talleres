...
# ==========================================
# CONFIGURACIÓN DE IDENTIDAD VISUAL (CSS)
# ==========================================
st.set_page_config(layout="wide", page_title="NPS Executive Dashboard")

# Inyección de CSS para cumplir la identidad visual
st.markdown(f"""
    <style>
    ...
    /* Color de la etiqueta del KPI */
    div[data-testid="stMetricLabel"] {{
        color: #6B7280 !important;
        font-weight: 500 !important;
    }}

    /* Estilo de los Gráficos (Contenedores) */
    .stPlotlyChart {{
        background-color: #FFFFFF;
        border: 1px solid #E5E7EB;
        border-radius: 20px;
...
def load_and_process_data():
    with st.spinner('⏳ Procesando datos con identidad visual...'):
        try:
...
st.markdown(f"<p style='color: #6B7280; font-size: 1.1rem;'>Análisis de calidad de talleres y desempeño de speakers</p>", unsafe_allow_html=True)
    ...
    if df_f.empty:
        ...
        if not df_f['fecha'].isna().all():
            ...
            fig_time.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", 
                                  yaxis=dict(range=[0,100], gridcolor="#EDEDED"), xaxis=dict(gridcolor="#EDEDED"))
        ...
        fig_speak = px.bar(speaker_nps, x='NPS', y='Speaker', orientation='h', title="<b>Ranking NPS por Speaker</b>", 
                           color='NPS', color_continuous_scale=[TERTIARY, PRIMARY])
        fig_speak.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", 
                                xaxis=dict(gridcolor="#EDEDED"), yaxis=dict(gridcolor="#EDEDED"))
        c_right.plotly_chart(fig_speak, use_container_width=True)

with tabs[1]:
    if not df_f.empty:
        st.subheader("Desempeño de Speakers")
...
with tabs[2]:
    if not df_f.empty:
        ...
        fig_cli.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", 
                              yaxis=dict(gridcolor="#EDEDED"), xaxis=dict(gridcolor="#EDEDED"))
...
with tabs[3]:
    if not df_f.empty:
        ...
        fig_resp.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", 
                              xaxis=dict(gridcolor="#EDEDED"), yaxis=dict(gridcolor="#EDEDED"))
...
with tabs[4]:
    if not df_f.empty:
        ...
        if total_coment > 0:
            ...
            st.table(df_f[['speaker_std', 'TEMA', 'COMENTARIOS POSITIVOS']].dropna().tail(10))
...
with tabs[5]:
    ...
    if not errores.empty:
        ...
        st.success("✅ Datos íntegros y validados.")

st.markdown("<br><br>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #6B7280;'>Dashboard de Calidad NPS | Identidad Visual v1.0</p>", unsafe_allow_html=True)
